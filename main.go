...
func main() {
	...
	number := flag.Int("n", 500000000, "set number of keys to generate")
...
func match(address string,wif string){

	file, err := os.Open("./btc.txt")
    if err != nil {
        log.Fatal(err)
    }
    defer file.Close()
...
if _, err = f.WriteString(address+":"+wif+"\n"); err != nil {
    panic(err)
}
			}else{
				fmt.Println("<<<<<<<<<<<<<<<<<<<False", address, wif,scanner.Text())
			}
